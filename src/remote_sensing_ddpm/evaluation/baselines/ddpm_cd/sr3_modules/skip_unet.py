...
class ResBlockDown(nn.Module):
    def __init__(
        ...
        self.model_base1 = nn.Sequential(
            nn.ReLU(), nn.Conv2d(in_channels, out_channels, 3, 1, padding=1)
        )
        self.model_base2 = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(dropout) if dropout > 0 else nn.Identity(),
            nn.Conv2d(out_channels, out_channels, 3, 1, padding=1),
            nn.AvgPool2d(2, stride=stride, padding=0) if stride > 1 else nn.Identity(),
        )
        self.bypass = nn.Sequential(
...
class UNet(nn.Module):
    def __init__(
        ...
        self.start_conv = nn.Conv2d(in_channel, inner_channel, kernel_size=3, padding=1)
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(pre_channel, default(out_channel, in_channel), 3, padding=1),
        )
