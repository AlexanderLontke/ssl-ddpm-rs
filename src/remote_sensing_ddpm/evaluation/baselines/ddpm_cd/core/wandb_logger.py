...
class WandbLogger:
    ...
    def __init__(self, opt):
        ...
        if self._wandb.run is None:
            self._wandb.init(
                project=opt["wandb"]["project"], config=opt, dir="./experiments"
            )

        self.config = self._wandb.config

        if self.config.get("log_eval", None):
            self.eval_table = self._wandb.Table(
                columns=["fake_image", "sr_image", "hr_image", "psnr", "ssim"]
            )
        else:
            self.eval_table = None

        if self.config.get("log_infer", None):
            self.infer_table = self._wandb.Table(
                columns=["fake_image", "sr_image", "hr_image"]
            )
        else:
    ...
    def log_checkpoint(self, current_epoch, current_step):
        ...
        gen_path = os.path.join(
            ...
            "I{}_E{}_gen.pth".format(current_step, current_epoch),
        )
        opt_path = os.path.join(
            self.config.path["checkpoint"],
            "I{}_E{}_opt.pth".format(current_step, current_epoch),
        )
    ...
    def log_eval_data(self, sam, psnr=None, ssim=None):
        ...
        if psnr is not None and ssim is not None:
            self.eval_table.add_data(sam, psnr, ssim)
        else:
            self.infer_table.add_data(self._wandb.Image(sam))
