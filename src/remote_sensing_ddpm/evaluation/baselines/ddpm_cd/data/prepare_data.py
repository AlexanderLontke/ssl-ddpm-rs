...
def prepare_process_worker(wctx, file_subset):
    for file in file_subset:
        ...
        if not wctx.lmdb_save:
            ...
            sr_img.save(
                "{}/sr_{}_{}/{}.png".format(
                    wctx.out_path, wctx.sizes[0], wctx.sizes[1], i.zfill(5)
                )
            )
        else:
            with wctx.env.begin(write=True) as txn:
                txn.put(
                    "lr_{}_{}".format(wctx.sizes[0], i.zfill(5)).encode("utf-8"), lr_img
                )
                txn.put(
                    "hr_{}_{}".format(wctx.sizes[1], i.zfill(5)).encode("utf-8"), hr_img
                )
                txn.put(
                    "sr_{}_{}_{}".format(
                        wctx.sizes[0], wctx.sizes[1], i.zfill(5)
                    ).encode("utf-8"),
                    sr_img,
                )
...
def prepare(
    ...
    resize_fn = partial(
        resize_worker, sizes=sizes, resample=resample, lmdb_save=lmdb_save
    )
    ...
    if n_worker > 1:
        ...
        for file in tqdm(files):
            ...
            if not lmdb_save:
                ...
                sr_img.save(
                    "{}/sr_{}_{}/{}.png".format(
                        out_path, sizes[0], sizes[1], i.zfill(5)
                    )
                )
            else:
                with env.begin(write=True) as txn:
                    txn.put(
                        "lr_{}_{}".format(sizes[0], i.zfill(5)).encode("utf-8"), lr_img
                    )
                    txn.put(
                        "hr_{}_{}".format(sizes[1], i.zfill(5)).encode("utf-8"), hr_img
                    )
                    txn.put(
                        "sr_{}_{}_{}".format(sizes[0], sizes[1], i.zfill(5)).encode(
                            "utf-8"
                        ),
                        sr_img,
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", "-p", type=str, default="{}/Dataset/celebahq_256".format(Path.home())
    )
