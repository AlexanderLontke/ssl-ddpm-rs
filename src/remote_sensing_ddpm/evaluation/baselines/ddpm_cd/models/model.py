...
class DDPM(BaseModel):
    ...
    def get_single_representation(self, t):
        self.netG.eval()
        img = self.data["image"]
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
    ...
    def load_network(self):
        ...
        if load_path is not None:
            ...
            network.load_state_dict(
                torch.load(gen_path, map_location=torch.device(self.device)),
                strict=True,
            )

            if self.opt["phase"] == "train":
                # optimizer
                opt = torch.load(opt_path, map_location=torch.device(self.device))
