...
class cd_head(nn.Module):
    ...
    def __init__(
        ...
        for feat in feat_scales:
            if psp:
                self.diff_layers.append(
                    _PSPModule(
                        in_channels=get_in_channels(
                            [feat], inner_channel, channel_multiplier
                        ),
                        bin_sizes=[1, 2, 3, 6],
                    )
                )
    ...
    def forward(self, feats_A, feats_B):
        ...
        for layer in self.diff_layers:
            x = layer(
                torch.abs(feats_A[self.feat_scales[c]] - feats_B[self.feat_scales[c]])
            )
        ...
        for i in range(0, len(feats_diff)):
            if feats_diff[i].size(2) != self.img_size:
                feat_diff_up = F.interpolate(
                    feats_diff[i], size=(self.img_size, self.img_size), mode="bilinear"
                )
            else:
