    ...
    if opt["phase"] == "train":
        for current_epoch in range(start_epoch, n_epoch):
            ...
            for current_step, train_data in enumerate(train_loader):
                ...
                for t in opt["model_cd"]["t"]:
                    fe_A_t, fd_A_t, fe_B_t, fd_B_t = diffusion.get_feats(
                        t=t
                ...
                if current_step % opt["train"]["train_print_freq"] == 0:
                    ...
                    if img_mode == "single":
                        # Converting to uint8
                        img_A = Metrics.tensor2img(
                            train_data["A"], out_type=np.uint8, min_max=(-1, 1)
                        )  # uint8
                        img_B = Metrics.tensor2img(
                            train_data["B"], out_type=np.uint8, min_max=(-1, 1)
                        )  # uint8
                        ...
                        Metrics.save_img(
                            img_A,
                            "{}/img_A_e{}_b{}.png".format(
                                train_result_path, current_epoch, current_step
                            ),
                        )
                        Metrics.save_img(
                            img_B,
                            "{}/img_B_e{}_b{}.png".format(
                                train_result_path, current_epoch, current_step
                            ),
                        )
                        Metrics.save_img(
                            pred_cm,
                            "{}/img_pred_e{}_b{}.png".format(
                                train_result_path, current_epoch, current_step
                            ),
                        )
                        Metrics.save_img(
                            gt_cm,
                            "{}/img_gt_e{}_b{}.png".format(
                                train_result_path, current_epoch, current_step
                            ),
                        )
                    else:
            ...
            if current_epoch % opt["train"]["val_freq"] == 0:
                ...
                for current_step, val_data in enumerate(val_loader):
                    ...
                    if current_step % opt["train"]["val_print_freq"] == 0:
                        ...
                        if img_mode == "single":
                            # Converting to uint8
                            img_A = Metrics.tensor2img(
                                val_data["A"], out_type=np.uint8, min_max=(-1, 1)
                            )  # uint8
                            img_B = Metrics.tensor2img(
                                val_data["B"], out_type=np.uint8, min_max=(-1, 1)
                            )  # uint8
                            gt_cm = Metrics.tensor2img(
                                visuals["gt_cm"].unsqueeze(1).repeat(1, 3, 1, 1),
                                out_type=np.uint8,
                ...
                change_detection.save_network(
                    current_epoch, is_best_model=is_best_model
                )
                change_detection._clear_cache()
        ...
        for current_step, test_data in enumerate(test_loader):
            ...
            if img_mode == "single":
                ...
                visuals["gt_cm"] = visuals["gt_cm"] * 2.0 - 1.0
                img_A = Metrics.tensor2img(
                    test_data["A"], out_type=np.uint8, min_max=(-1, 1)
                )  # uint8
                img_B = Metrics.tensor2img(
                    test_data["B"], out_type=np.uint8, min_max=(-1, 1)
                )  # uint8
                gt_cm = Metrics.tensor2img(
                    visuals["gt_cm"].unsqueeze(1).repeat(1, 3, 1, 1),
                    out_type=np.uint8,
                ...
                Metrics.save_img(
                    pred_cm,
                    "{}/img_pred_cm{}.png".format(test_result_path, current_step),
                )
                Metrics.save_img(
